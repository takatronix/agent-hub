...
- `claude`: `claude -p --output-format stream-json`。**既定モデルは `opus` にしておくこと**（アカウント既定が Fable だと Max の利用枠を急速に消費する）。run ごとに UI / MCP の `models` で opus / fable に上げられる。Max/Pro ログインなら API 課金は発生せず、UI の `$` 表示は推定値。既定で `--dangerously-skip-permissions`（`"skip_permissions": false` で外せる）。複数アカウントは `CLAUDE_CONFIG_DIR` を変えた agent を複数並べるだけ。
