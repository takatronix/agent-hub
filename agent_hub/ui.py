...
.ag{display:flex;gap:10px;align-items:center;padding:8px 10px;border-radius:12px;border:1px solid transparent;cursor:pointer;transition:.18s;user-select:none;animation:slidein .35s both}
...
const KIND={claude:['Claude Code','ファイル・コマンド OK'],codex:['Codex CLI','ファイル・コマンド OK'],command:['CLI','ファイル・コマンド OK'],kimi:['Kimi','ファイル・コマンド OK'],api:['API / ローカル LLM','テキストのみ'],fake:['ダミー','配線テスト用'],hub:['hub','']};
...
async function showHome(){$('#crumb').textContent='';const [a,r]=await Promise.all([api('/api/agents'),api('/api/runs?limit=100')]);agents=a.agents;selected=selected.filter(n=>agents.some(x=>x.name===n&&x.online));
 ...
 <section>
  <div class="card"><h2>新しいミッション</h2><div class="sub">課題を書く → 誰に頼むか選ぶ → 開始。AI 同士のやりとりは全部記録されます</div>
   ...
   <div class="step"><span class="n">2</span><h3>誰に頼むか</h3><span>左のエージェントをタップ</span></div>
 ...
 renderAgents();renderRuns(r.runs);renderRecipes();updateForm();
 $('#start').onclick=start;
...
function renderAgents(){const el=$('#agents');if(!el)return;if(!agents.length){el.innerHTML='<div class="empty">runner が未接続</div>';return}
 ...
 el.innerHTML=hosts.map(h=>`<div class="host"><div class="hn"><span>🖥 ${esc(h)}</span><span>${agents.filter(a=>a.host===h&&a.online).length}/${agents.filter(a=>a.host===h).length}</span></div>`+agents.filter(a=>(a.host||'?')===h).map(a=>{const on=a.online,sel=selected.includes(a.name),k=KIND[a.kind]||[a.kind,''];
  return`<div class="ag ${sel?'sel':''} ${on?'':'off'}" style="animation-delay:${(i++)*40}ms" onclick="${on?`toggleAgent('${a.name}')`:''}">${av(a.name)}<div style="min-width:0"><div class="nm">${esc(a.name)}</div><div class="ds">${esc(k[0])}${a.meta&&a.meta.model?' · '+esc(a.meta.model):''}</div></div>${sel?'<span class="chk">✓</span>':on?`<span class="dot ${a.status==='busy'?'busy':'on'}" title="${a.status==='busy'?'作業中':'待機中'}"></span>`:`<a href="#" class="chk" style="color:var(--dim)" onclick="event.stopPropagation();forgetAgent('${a.name}');return false">✕</a>`}</div>`}).join('')+`</div>`).join('')}
...
function renderRun(){const ph=PHASES[RUN.recipe]||[];const cur=RUN.state.phase;const ci=ph.findIndex(p=>p[0]===cur);const finished=RUN.status!=='running';
 ...
 const cost=RUN.tasks.reduce((s,t)=>s+((t.meta.usage||{}).total_cost_usd||0),0);const dur=RUN.tasks.reduce((s,t)=>s+(t.meta.duration_s||0),0);
 $('#stat').innerHTML=`<span>参加 <b>${[...new Set(RUN.tasks.map(t=>t.agent))].length}</b></span><span>タスク <b>${RUN.tasks.filter(t=>t.status==='done').length}/${RUN.tasks.length}</b></span><span>AI 稼働 <b>${Math.round(dur)}s</b></span>${cost?`<span>Claude <b>$${cost.toFixed(2)}</b></span>`:''}`;
...
function colHead(t){const meta=t.meta||{};return`<h3>${av(t.agent)}<span class="t-${kindOf(t.agent)}" style="font-weight:700">${esc(t.agent)}</span>${meta.model?`<span class="tiny mono">${esc(meta.model)}</span>`:''}${pill(t.status)}<span class="tiny" style="margin-left:auto">${meta.duration_s?meta.duration_s+'s':''}${meta.usage&&meta.usage.total_cost_usd!=null?' · $'+meta.usage.total_cost_usd.toFixed(2):''}</span></h3>`}
