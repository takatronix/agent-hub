...
function renderRun(){const ph=PHASES[RUN.recipe]||[];const cur=RUN.state.phase;const ci=ph.findIndex(p=>p[0]===cur);const finished=RUN.status!=='running';
 ...
 $('#stat').innerHTML=`<span>参加 <b>${[...new Set(RUN.tasks.map(t=>t.agent))].length}</b></span><span>タスク <b>${RUN.tasks.filter(t=>t.status==='done').length}/${RUN.tasks.length}</b></span><span>AI 稼働 <b>${Math.round(dur)}s</b></span>${cost?`<span title="Claude Code の推定額。Max/Pro サブスクでログインしている場合は課金されません（利用枠を消費）">推定 <b>$${cost.toFixed(2)}</b> <span class="tiny">(Max は課金なし)</span></span>`:''}`;
