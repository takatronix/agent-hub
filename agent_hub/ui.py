...
function renderRun(){const ph=PHASES[RUN.recipe]||[];const cur=RUN.state.phase;const ci=ph.findIndex(p=>p[0]===cur);const finished=RUN.status!=='running';
 ...
 $('#stat').innerHTML=`<span>参加 <b>${[...new Set(RUN.tasks.map(t=>t.agent))].length}</b></span><span>タスク <b>${RUN.tasks.filter(t=>t.status==='done').length}/${RUN.tasks.length}</b></span><span>AI 稼働 <b>${Math.round(dur)}s</b></span>${costApi?`<span title="API 従量課金（Grok など）の実費">API 実費 <b>$${costApi.toFixed(3)}</b></span>`:''}${costSub?`<span title="Claude Code の推定額。Max/Pro サブスクなら課金されず利用枠を消費">Claude 推定 <b>$${costSub.toFixed(2)}</b> <span class="tiny">(枠)</span></span>`:''}`;
