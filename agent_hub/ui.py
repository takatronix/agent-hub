...
.settled .card,.settled .col,.settled .run,.settled .ag{animation:none}
...
<header><a class="logo" href="/" onclick="event.preventDefault();go('/')">agent-hub</a><span id="crumb" class="crumb"></span><span class="sp"></span><span id="live" class="live"><b></b>offline</span><span class="theme" id="bell" title="通知オン/オフ">🔔</span><span class="theme" id="theme" title="テーマ切替">☾</span></header>
...
let notifyOn=false;try{notifyOn=localStorage.getItem('notify')==='on'}catch(e){}
function syncBell(){const b=$('#bell');if(b)b.style.opacity=notifyOn?'1':'.4'}
(function(){document.addEventListener('DOMContentLoaded',()=>{syncBell();const b=$('#bell');if(!b)return;b.onclick=async()=>{if(!notifyOn){if(window.Notification){try{const p=await Notification.requestPermission();if(p!=='granted'){toast('通知が許可されませんでした');return}}catch(e){}}notifyOn=true;toast('完了時に通知します')}else{notifyOn=false;toast('通知をオフにしました')}try{localStorage.setItem('notify',notifyOn?'on':'off')}catch(e){}syncBell()}})})();
const notifiedRuns={};
function notifyRun(e){if(!notifyOn||!window.Notification||Notification.permission!=='granted')return;let r;try{r=JSON.parse(e.data).run}catch(err){return}if(!r||(r.status!=='done'&&r.status!=='failed'))return;if(notifiedRuns[r.id]===r.status)return;notifiedRuns[r.id]=r.status;try{new Notification((r.status==='done'?'✅ ':'❌ ')+(r.title||'ミッション'),{body:String(r.summary||'').slice(0,120)})}catch(err){}}
...
const ACTVERB={Read:['📖','を読む'],Edit:['✏','を編集'],Write:['📝','を書く'],Bash:['⚙','を実行'],Grep:['🔍','を検索'],Glob:['🔍','を検索'],Task:['🤝','に委任'],WebFetch:['🌐','を取得'],WebSearch:['🌐','を検索']};
function activityOf(msg){if(!msg)return'';if(msg.role==='thinking')return'🤔 考え中';if(msg.role!=='tool_use')return'';const d=msg.data||{};const name=d.name||'';const inp=d.input||{};const v=ACTVERB[name]||['🔧','を実行'];const arg=String(inp.file_path||inp.command||inp.pattern||inp.query||'').split('\n')[0];const s=arg.length>48?arg.slice(0,48)+'…':arg;return `${v[0]} ${s?esc(s)+' ':''}${v[1]}`}
const ACTIVITY={};const RUNACT={};
let agents=[],es=null;
...
async function showHome(){runsKey='';agentsKey='';schedKey='';$('#crumb').textContent='';const [a,r,sc]=await Promise.all([api('/api/agents'),api('/api/runs?limit=100'),api('/api/schedules').catch(()=>({schedules:[]}))]);agents=a.agents;selected=selected.filter(n=>agents.some(x=>x.name===n&&x.online));
 ...
 <section>
  <div class="card"><h2>新しいミッション</h2><div class="sub">課題を書く → 誰に頼むか選ぶ → 開始。AI 同士のやりとりは全部記録されます</div>
   ...
   <div class="row" id="synthrow" style="margin-top:10px"><label style="flex:0 1 420px"><span class="tiny" id="synthlabel">統合役</span><select id="synth"></select></label><label id="anglesrow" style="flex:0 1 auto;display:flex;gap:6px;align-items:center;cursor:pointer"><input type="checkbox" id="autoangles" checked style="width:auto"> <span class="tiny">配役を AI に任せる（個性・実績から担当視点を割り当て）</span></label></div>
   <div class="row" id="schedrow" style="margin-top:10px"><label style="flex:0 1 auto;display:flex;gap:6px;align-items:center;cursor:pointer"><input type="checkbox" id="schedon" onchange="updateForm()" style="width:auto"> <span class="tiny">⏰ 定期実行（この課題を繰り返し自動実行）</span></label><label id="schedint" style="flex:0 1 auto;display:none;gap:6px;align-items:center"><span class="tiny">間隔</span><select id="interval" style="width:auto;flex:0 0 auto"><option value="1800">30分ごと</option><option value="3600" selected>1時間ごと</option><option value="21600">6時間ごと</option><option value="86400">毎日</option></select></label></div>
  ...
  <div class="card"><h2>定期ミッション <span class="tiny">⏰ 自動実行</span></h2><div class="sub">決めた間隔で自動的に走るミッション。上のフォームで「⏰ 定期実行」を ON にすると登録できます</div><div id="schedules"></div></div>
 ...
 renderAgents();renderRuns(r.runs);renderSchedules(sc.schedules);renderRecipes();updateForm();renderBoard();
 $('#start').onclick=start;
 ...
 const s=connect(null);s.addEventListener('message',e=>{let m;try{m=JSON.parse(e.data).message}catch(err){return}const a=activityOf(m);if(a&&m&&m.run_id){RUNACT[m.run_id]=a;const line=document.querySelector(`.run[data-run="${m.run_id}"] .act`);if(line)line.innerHTML=a}});s.addEventListener('run',async e=>{notifyRun(e);renderRuns((await api('/api/runs?limit=100')).runs);renderSchedules((await api('/api/schedules').catch(()=>({schedules:[]}))).schedules)});s.addEventListener('agent',async()=>{agents=(await api('/api/agents')).agents;renderAgents()})}
...
function updateForm(){$('#synthrow').style.display=(recipe==='review_panel'||recipe==='team')?'flex':'none';$('#synthlabel').textContent=recipe==='team'?'統合役（分担も決める。Claude / Codex 推奨）':'統合役（最終回答をまとめる人。Claude / Codex 推奨）';$('#anglesrow').style.display=recipe==='review_panel'?'flex':'none';const sy=$('#synth');const cur=sy.value;sy.innerHTML=selected.map(n=>`<option value="${n}">${n}</option>`).join('');if(selected.includes(cur))sy.value=cur;
 ...
 const sched=$('#schedon')&&$('#schedon').checked;if($('#schedint'))$('#schedint').style.display=sched?'flex':'none';
 const need=(recipe==='review_panel'||recipe==='team')?2:1;$('#start').disabled=selected.length<need;if($('#start'))$('#start').textContent=sched?'⏰ 定期登録':'🚀 開始';$('#hint').textContent=selected.length<need?`あと ${need-selected.length} 人選んでください`:(sched?'この課題を定期実行に登録します':(recipe==='single'?`${selected[0]} に頑みます`:`${selected.length} 人で実行します`))}
...
async function renderBoard(){const el=$('#board');if(!el)return;const j=await api('/api/leaderboard');const b=j.leaderboard;const names=Object.keys(b);if(!names.length){el.innerHTML='<div class="empty">まだ採点データがありません。三者評価を 1 回走らせると埋まります</div>';return}
 ...
 el.innerHTML=`<div style="overflow-x:auto"><table style="border-collapse:collapse;width:100%;font-size:13px"><thead><tr><th style="text-align:left;padding:6px 8px;color:var(--mut);font-weight:500">エージェント</th><th style="text-align:left;padding:6px 8px;color:var(--mut);font-weight:500">総合</th>${cats.map(c=>`<th style="text-align:left;padding:6px 8px;color:var(--mut);font-weight:500">${CAT_JA[c]||c}</th>`).join('')}</tr></thead><tbody>${names.map(n=>`<tr style="border-top:1px solid var(--line)"><td style="padding:6px 8px;white-space:nowrap">${av(n)} <b class="t-${kindOf(n)}">${esc(n)}</b></td><td style="padding:6px 8px">${cell(b[n].all)}</td>${cats.map(c=>`<td style="padding:6px 8px">${cell(b[n][c])}</td>`).join('')}</tr>`).join('')}</tbody></table></div><div class="tiny" style="margin-top:6px">平均点 /件数 ・ ★ 最良に選ばれた回数 ・ ✔ 統合で採用された回数 ・ ⏱ 解答の平均時間</div>`}
let runsKey='';
function renderRuns(runs){const el=$('#runs');if(!el)return;const k=runs.map(r=>r.id+r.status+(r.state.phase||'')).join();if(k===runsKey)return;runsKey=k;if(!runs.length){el.innerHTML='<div class="empty">まだミッションがありません</div>';return}
 el.innerHTML=runs.map((r,i)=>{const parts=r.spec.solvers||r.spec.members||r.spec.agents||(r.spec.agent?[r.spec.agent]:[]);const ph=(PHASES[r.recipe]||[]).find(p=>p[0]===r.state.phase);const act=r.status==='running'?`<div class="me act tiny" style="opacity:.7;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${RUNACT[r.id]||''}</div>`:'';
  return`<div class="run" data-run="${r.id}" style="animation-delay:${i*30}ms" onclick="go('/runs/${r.id}')"><div>${pill(r.status)}</div><div style="min-width:0"><div class="ti">${esc(r.title)}</div><div class="me">${RECIPES.find(x=>x[0]===r.recipe)?.[1]||r.recipe}${r.status==='running'&&ph?' · '+ph[1]+'中':''} · ${esc(r.project)} · ${fmtDt(r.created_at)}</div>${act}</div><div class="avs">${parts.map(n=>av(n)).join('')}</div></div>`}).join('')}
async function start(){const prompt=$('#prompt').value.trim();if(!prompt)return toast('課題を書いてください');
 const spec={prompt,workdir:$('#workdir').value||null,models:Object.fromEntries(selected.filter(n=>models[n]).map(n=>[n,models[n]]))};if(recipe==='review_panel'){spec.solvers=selected;spec.synthesizer=$('#synth').value||selected[0];spec.auto_angles=$('#autoangles').checked&&selected.length>=3}else if(recipe==='team'){spec.members=selected;spec.integrator=$('#synth').value||selected[0]}else if(recipe==='parallel')spec.agents=selected;else spec.agent=selected[0];
 const title=prompt.split('\n')[0].slice(0,70),project=$('#project').value||'default';
 if($('#schedon')&&$('#schedon').checked){const interval_sec=parseInt($('#interval').value,10);try{await api('/api/schedules',{title,recipe,interval_sec,project,spec,notify:'on_report'});toast('定期ミッションを登録しました');renderSchedules((await api('/api/schedules').catch(()=>({schedules:[]}))).schedules)}catch(e){toast(e.message)}return}
 try{const j=await api('/api/runs',{recipe,project,title,spec,created_by:'web'});toast('開始しました');go('/runs/'+j.run.id)}catch(e){toast(e.message)}}
const SCHEDINT={1800:'30分ごと',3600:'1時間ごと',21600:'6時間ごと',86400:'毎日'};
let schedKey='';
function renderSchedules(list){const el=$('#schedules');if(!el)return;list=list||[];const k=list.map(s=>s.id+s.enabled+s.next_at+(s.last_run_id||'')).join();if(k===schedKey)return;schedKey=k;if(!list.length){el.innerHTML='<div class="empty">定期ミッションはまだありません</div>';return}
 el.innerHTML=list.map(s=>{const rn=RECIPES.find(x=>x[0]===s.recipe)?.[1]||s.recipe;const iv=SCHEDINT[s.interval_sec]||(s.interval_sec+'秒ごと');const en=s.enabled?'<span class="pill done">ON</span>':'<span class="pill">OFF</span>';const last=s.last_run_id?` · <a href="/runs/${jesc(s.last_run_id)}" onclick="event.preventDefault();go('/runs/${jesc(s.last_run_id)}')">最新結果</a>`:'';
  return`<div class="run" style="cursor:default"><div>${en}</div><div style="min-width:0"><div class="ti">⏰ ${esc(s.title)}</div><div class="me">${esc(rn)} · ${esc(iv)} · 次回 ${fmtDt(s.next_at)} · ${esc(s.project)}${last}</div></div><div style="display:flex;gap:6px"><button class="ghost" style="padding:4px 10px;font-size:12px" title="有効/無効" onclick="schedToggle('${jesc(s.id)}')">⏯</button><button class="ghost" style="padding:4px 10px;font-size:12px" title="今すぐ実行" onclick="schedRun('${jesc(s.id)}')">▶</button><button class="ghost" style="padding:4px 10px;font-size:12px" title="削除" onclick="schedDel('${jesc(s.id)}')">✕</button></div></div>`}).join('')}
async function schedToggle(id){try{await api('/api/schedules/'+encodeURIComponent(id)+'/toggle',{});renderSchedules((await api('/api/schedules')).schedules)}catch(e){toast(e.message)}}
async function schedRun(id){try{const j=await api('/api/schedules/'+encodeURIComponent(id)+'/run_now',{});toast('実行しました');go('/runs/'+j.run.id)}catch(e){toast(e.message)}}
async function schedDel(id){if(!confirm('この定期ミッションを削除しますか？'))return;try{await api('/api/schedules/'+encodeURIComponent(id)+'/delete',{});renderSchedules((await api('/api/schedules')).schedules);toast('削除しました')}catch(e){toast(e.message)}}
/* ---------------- run ---------------- */
...
async function showRun(id){childrenLoaded=false;bodyKey='';agents=(await api('/api/agents')).agents;const j=await api('/api/runs/'+id);RUN=j.run;MSGS=(await api(`/api/runs/${id}/messages`)).messages;lastId=MSGS.length?MSGS[MSGS.length-1].id:0;view=RUN.status==='done'?'results':'cols';
 ...
 renderRun();
 MSGS.forEach(m=>{const a=activityOf(m);if(a&&m.task_id)ACTIVITY[m.task_id]=a});
 const resync=async()=>{try{const j2=await api('/api/runs/'+id);RUN=j2.run;const mm=(await api(`/api/runs/${id}/messages?after=${lastId}`)).messages;for(const m of mm){if(m.id>lastId){MSGS.push(m);lastId=m.id;const a=activityOf(m);if(a&&m.task_id)ACTIVITY[m.task_id]=a}}renderRun()}catch(e){}};
 const s=connect(id,resync);s.addEventListener('message',e=>{const m=JSON.parse(e.data).message;if(m.id>lastId){MSGS.push(m);lastId=m.id;const a=activityOf(m);if(a&&m.task_id){ACTIVITY[m.task_id]=a;updateActivity(m.task_id)}appendMsg(m)}});
...
function renderRun(){const ph=PHASES[RUN.recipe]||[];const cur=RUN.state.phase;const ci=ph.findIndex(p=>p[0]===cur);const finished=RUN.status!=='running';
 ...
 $('#stat').innerHTML=`<span>参加 <b>${[...new Set(RUN.tasks.map(t=>t.agent))].length}</b></span>${RUN.state.category?`<span>カテゴリ <b>${CAT_JA[RUN.state.category]||RUN.state.category}</b></span>`:''}${RUN.state.review_avg?`<span>レビュー平均 <b>${RUN.state.review_avg}点</b></span>`:''}<span>タスク <b>${RUN.tasks.filter(t=>t.status==='done').length}/${RUN.tasks.length}</b></span><span>AI 稼働 <b>${Math.round(dur)}s</b></span>${timeStat()}${costApi?`<span title="API 従量課金（Grok など）の実費">API 実費 <b>$${costApi.toFixed(3)}</b></span>`:''}${costSub?`<span title="Claude Code の推定額。Max/Pro サブスクなら課金されず利用枠を消費">Claude 推定 <b>$${costSub.toFixed(2)}</b> <span class="tiny">(枠)</span></span>`:''}`;
...
function followTargets(){const parts=[...new Set(RUN.tasks.map(t=>t.agent))];const rest=agents.filter(a=>a.online&&!parts.includes(a.name)).map(a=>a.name);const synth=RUN.spec.synthesizer||RUN.spec.integrator||RUN.spec.agent||parts[0];return {synth,list:[...parts.filter(n=>agents.some(a=>a.name===n&&a.online)),...rest]}}
...
function colHead(t){const meta=t.meta||{};const al=aliasOf(t.agent);const ang=meta.angle?`<span class="tiny" title="${esc(meta.angle)}" style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">🎯 ${esc(meta.angle.split('\n')[0])}</span>`:'';return`<h3>${av(t.agent)}<span class="t-${kindOf(t.agent)}" style="font-weight:700">${esc(t.agent)}</span>${al&&t.step==='solve'?`<span class="tiny mono" title="レビュー時の匿名記号">回答 ${al}</span>`:''}${t.step==='solve'?scoreOf(t.agent):''}${ang}${meta.model?`<span class="tiny mono">${esc(meta.model)}</span>`:''}${pill(t.status)}<span class="tiny" style="margin-left:auto">${meta.duration_s?meta.duration_s+'s':''}${meta.usage&&meta.usage.total_cost_usd!=null?' · $'+meta.usage.total_cost_usd.toFixed(2):''}</span></h3>${t.status==='running'&&ACTIVITY[t.id]?`<div class="tiny act" data-act="${t.id}" style="padding:2px 14px;color:var(--mut);opacity:.75;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${ACTIVITY[t.id]}</div>`:''}`}
function updateActivity(taskId){const line=document.querySelector(`.col[data-task="${taskId}"] .act`);if(line){line.innerHTML=ACTIVITY[taskId]||'';return}const col=document.querySelector(`.col[data-task="${taskId}"] h3`);if(col&&ACTIVITY[taskId]){col.insertAdjacentHTML('afterend',`<div class="tiny act" data-act="${taskId}" style="padding:2px 14px;color:var(--mut);opacity:.75;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${ACTIVITY[taskId]}</div>`)}}
